...
| `glpi_plugin_taskprocedure_ticketsteps` | Snapshot das etapas executáveis, incluindo conclusão, comentário e evidência |
| `glpi_plugin_taskprocedure_step_logs` | Histórico de conclusão, detalhes e ordenação das etapas |
...
As colunas `comment` e `evidence` pertencem à instância da etapa, enquanto o histórico registra mudanças com usuário, ação, valor anterior e novo valor. A atualização do schema é idempotente para instalações existentes.
