...
Após a instalação, confirmar no banco a existência das cinco tabelas. Em uma instalação limpa, as tabelas devem estar vazias. Abrir um chamado e confirmar a aba lateral **Procedimentos**, com a mensagem “Nenhum procedimento associado a este chamado.” quando não houver associação.
...
Com um procedimento associado a um chamado, confirmar que cada etapa aparece como uma caixa de seleção. Marcar e desmarcar uma etapa deve atualizar `is_completed`, `completed_by` e `completed_at` em `glpi_plugin_taskprocedure_ticketsteps`, além de registrar a alteração em `glpi_plugin_taskprocedure_step_logs`, sem permitir alterar uma etapa de outro chamado ou procedimento. Também validar comentários, evidências, reordenação e o progresso visual.
